import { validateCreateRegistroDiario, validateUpdateRegistroDiario } from '../registroDiario.validator';
...
describe('validateCreateRegistroDiario', () => {
    ...
    it('debe rechazar nivelEnergia menor a 1', () => {
        ...
        expect(res.json).toHaveBeenCalledWith({
            errors: expect.arrayContaining(['El nivel de energía es obligatorio y debe ser un número entre 1 y 5']),
        });
    });
    ...
    it('debe validar habitosCompletados si se envía', () => {
        const req = mockReq({
            fecha: '2026-07-20',
            nivelEnergia: 3,
            habitosCompletados: [{ habito: '507f1f77bcf86cd799439011', completado: true }],
        });
        const res = mockRes();
        validateCreateRegistroDiario(req, res, mockNext);
        expect(mockNext).toHaveBeenCalled();
    ...
    it('debe rechazar habitoCompletado con ObjectId inválido', () => {
        ...
        expect(res.json).toHaveBeenCalledWith({
            errors: expect.arrayContaining([
                'Hábito completado 1: el ID del hábito es obligatorio y debe ser un ObjectId valido',
            ]),
        });
    });
});
...
describe('validateUpdateRegistroDiario', () => {
    ...
    it('debe rechazar nivelEnergia fuera de rango', () => {
        ...
        validateUpdateRegistroDiario(req, res, mockNext);
        expect(res.json).toHaveBeenCalledWith({
            errors: expect.arrayContaining(['El nivel de energía debe ser un número entre 1 y 5']),
        });
    });
});
